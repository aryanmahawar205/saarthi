...
- **Recon / Discovery**: Identifies what the application is and its surface area.
- **SAST / DAST**: Discovers potential vulnerabilities from source code and runtime behavior.
- **Outputs**: Discovered endpoints, raw incidents, call graphs.
...
- **SecurityKnowledgeGraphAgent**: Combines attack surfaces, trust boundaries, call chains, and incidents into a formalized set of `nodes` and `edges`. This graph provides contextual relationships between an endpoint, a controller method, and an identified vulnerability.
